    ...
    <div class="py-6">
        ...
        @if ($posts->isEmpty())
            ...
            <div class="flex flex-col items-center justify-center text-center mt-12">
                <img src="https://cdn-icons-png.flaticon.com/256/7259/7259540.png" alt="Cat Let's Go" class="w-48 h-48">
                <h2 class="text-xl font-bold text-gray-800 mt-4">No posts yet!</h2>
                <p class="text-gray-600">Why not create your first post and get started?</p>
            </div>
        @else
            <!-- Table Container -->
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <div class="flex space-x-2">
                                        ...
                                        <div x-data="{ openModal: false }" class="inline-block">
                                            <button type="button" class="text-red-600 hover:text-red-700 font-semibold transition duration-300 transform hover:scale-105" @click="openModal = true">Delete</button>
                                            <div x-show="openModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm" x-cloak>
                                                <div class="relative bg-white rounded-xl shadow-lg w-full max-w-md p-6 overflow-hidden">
                                                    <button type="button" @click="openModal = false" class="absolute top-3 right-3 text-gray-400 hover:text-gray-600 focus:outline-none">
                                                    ...
                                                    <div class="mt-6 flex justify-center space-x-4">
                                                        <button type="button" @click="openModal = false" class="bg-gray-200 hover:bg-gray-300 text-gray-800 font-medium py-2 px-6 rounded-lg shadow-md transition duration-300 transform hover:scale-105">Cancel</button>
                                                        <form action="/dashboard/posts/{{ $post->slug }}" method="POST" class="inline-block">
                                                            @method('delete')
                                                            @csrf
                                                            <button type="submit" class="bg-red-600 hover:bg-red-700 text-white font-medium py-2 px-6 rounded-lg shadow-md transition duration-300 transform hover:scale-105">Delete</button>
                                                        </form>
                                                    </div>
                                                </div>
                                            </div>
